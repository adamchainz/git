...
`git diff [<options>] <blob> <blob>`::
	...
	contents of two blob objects.  A blob can be named using
	the `<rev>:<path>` syntax described in the "SPECIFYING
	REVISIONS" section in linkgit:gitrevisions[7], e.g.
	`HEAD:README`.  Because the two blobs are named
	independently, this form can compare files with different
	paths, for example a file that was renamed and modified
	between two commits (see the examples below).
...
+
------------
$ git diff main:CHANGELOG.md topic:docs/changelog.md  <1>
------------
+
<1> Compare the file `CHANGELOG.md` at the tip of the `main` branch
    with the file `docs/changelog.md` at the tip of the `topic`
    branch, e.g. to compare a file that was renamed and modified
